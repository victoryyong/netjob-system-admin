...
<section>
  <el-table  
    ...
    <el-table-column label="操作" v-if="treeType === 'normal'" width="220">
      <template scope="scope">  
        <el-button  :disabled="editOprt" @click="showEdit(scope.row._level, scope.row)" class="el-button el-button--default el-button--small">
            编辑  
        </el-button >  
        <el-button  
          size="small"  
        ...
        <el-button   :disabled="addOprt||scope.row._level===1" @click="handleAdd(scope.row.id,scope.row._level,scope.row)" class="el-button el-button--success el-button--small">
            添加下级
        </el-button>  
      </template>  
    </el-table-column>  
...
// 是否默认展开所有树  
    ...
    // 格式化数据源  
      ...
      editOprt:function(){
        return !util.hasOprt(this,'admin.dict.edit');
      },
      deleteOprt:function(){
        return !util.hasOprt(this,'admin.dict.delete');
      } 
    },  
    methods: {
     showEdit:function(level,row){
         let expendNode = '';
         let expendDeep = '';
         if(row._parent){
        ...
        this.isInit=true;
        this.$emit('showEdit',level,row,[expendNode,expendDeep]);
     },
     handleAdd:function(parentId,level,row){
         let expendNode = '';
         let expendDeep = '';
         if(row._parent){
             expendNode = row._parent.id;
        ...
        this.isInit=true;
        this.$emit('handleAdd',parentId,level,[expendNode,expendDeep,parentId]);
     },
    // 显示行  
