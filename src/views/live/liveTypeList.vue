	...
	<section>
		...
		<el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
			<el-form :inline="true" :model="filters">
				...
				<el-form-item>
					<el-button type="primary" v-on:click="getLiveTypes()">查询</el-button>
				</el-form-item>
				<el-form-item>
		...
		<!--列表-->
		<el-table :data="LiveTypes" highlight-current-row v-loading="listLoading" style="width: 100%;" @selection-change="selsChange">
...
<script>
	...
	import { listLiveType,editLiveType,addLiveType,deleteLiveType} from '../../api/api';
	...
	export default {
		...
		methods: {
			...
			getLiveTypes() {
				...
				NProgress.start();
				listLiveType(para).then((body) => {
					this.listLoading = false;
					NProgress.done();
					if(body){
					 this.total = body.page.totalRows;
					 this.LiveTypes = body.list;
					}
				});
			},
			handleEdit(row){
			...
			deleteHandle(id){
				 this.$confirm('确认删除该记录吗?', '提示', {
			          ...
			          this.listLoading = true;
			          NProgress.start();
			          let para = { liveTypeIds: id };
			          deleteLiveType(para).then((body) => {
			            this.listLoading = false;
			            NProgress.done();
			            if(body){
			              this.$message({
		...
		computed:{
			editOprt:function(){
				return !util.hasOprt(this,'admin.liveType.edit');
			},
			deleteOprt:function(){
				return !util.hasOprt(this,'admin.liveType.delete');
			},
			addOprt:function(){
				return !util.hasOprt(this,'admin.liveType.add');
			}
		}
	}
