<template>
	<section>
		...
		<!--列表-->
		<el-table :data="persons" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
			<el-table-column type="selection" width="50">
			</el-table-column>
			<el-table-column prop="name" label="名称" >
			</el-table-column>
			<el-table-column prop="code" label="证件号码" >
			</el-table-column>
		...
		<el-dialog title="详情" v-model="detailFormVisible" :close-on-click-modal="true" size="small" :before-close="handleClose">
			<el-form :model="detailForm" label-width="120px" ref="detailForm">
				...
				<el-form-item label="证件号：" prop="title">
					{{detailForm.code}}
				</el-form-item>
...
<script>
	...
	import { listPersonAuth,authPerson} from '../../api/api';
	...
	export default {
		...
		methods: {
			...
			getPersons() {
				...
				NProgress.start();
				listPersonAuth(para).then((body) => {
					this.listLoading = false;
					NProgress.done();
					if(body){
					 this.total = body.page.totalRows;
					 this.persons = body.list;
					}
				});
			},
			//显示编辑界面
				...
				this.$refs.authForm.validate((valid) => {
					...
					if (valid) {
						...
						type: 'warning'
								...
								let para = {type:4, authIds: ids,status:this.status,content:this.authForm.content };
								authPerson(para).then((body) => {
		...
		computed: {  
	      authOprt:function(){
	        return !util.hasOprt(this,'admin.person.auth');
	      }
    	}
	}
