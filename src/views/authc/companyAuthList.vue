<template>
	<section>
		...
		<!--列表-->
		<el-table :data="companys" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
			<el-table-column type="selection" width="50">
			</el-table-column>
			<el-table-column prop="member" label="名称" >
			</el-table-column>
...
<script>
	...
	import { listCompanyAuth,authCompany} from '../../api/api';
	...
	export default {
		...
		methods: {
			...
			getCompanys() {
				...
				NProgress.start();
				listCompanyAuth(para).then((body) => {
					this.listLoading = false;
					NProgress.done();
					if(body){
					 this.total = body.page.totalRows;
					 this.companys = body.list;
					}
				});
			},
			//显示编辑界面
		...
		computed: {  
	      authOprt:function(){
	        return !util.hasOprt(this,'admin.com.auth');
	      }
    	}
	}
